...
{
    public class DapperOperator<T> //where T : class
    {
        /// <summary>
        /// 数据库连接字符串(默认保障用工)
        /// </summary>
        private string connectionString = ConfigService.GetConfiguration().GetConnectionString("DB_GUARANTEE");

        public DapperOperator(string connStringName)
        {
            if (!string.IsNullOrWhiteSpace(connStringName))
                connectionString = ConfigService.GetConfiguration().GetConnectionString(connStringName);
        }
